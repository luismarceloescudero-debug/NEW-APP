...
Correrlo **antes de commitear** cualquier cosa en `js/data/` o `js/parsers/`. Los cuatro:
...
**`verificar` y `auditar` responden preguntas distintas y ninguno reemplaza al otro.** El primero
compara contra una línea base congelada: detecta un número que **cambió**, nunca una fórmula que
estuvo **mal desde el día uno**. El segundo recalcula todo desde su definición y lo confronta con
lo que produjo el pipeline.
