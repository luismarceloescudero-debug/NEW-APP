...
/**
 * Período automático común para todas las fuentes temporales.
 *
 * Las cargas son el ancla del negocio: sin una carga no existe consumo que medir. El GPS
 * agrega actividad para calcularlo, pero no debe estirar el período ni recortarlo por su cuenta.
 * Cuando existen ambas fuentes se toman solamente los meses presentes en las dos y se elige el
 * tramo consecutivo más largo (en empate, el más reciente). Así enero-agosto queda como un
 * período completo y un hueco no se tapa inventando continuidad.
 */
export function periodosAnalisisAutomatico(cargas = [], gps = []) {
    const mesesCargas = new Set(cargas.map(mesDe).filter(Boolean));
    const mesesGps = new Set(gps.flatMap(mesesDeRegistro));
    const base = mesesCargas.size && mesesGps.size
        ? [...mesesCargas].filter(m => mesesGps.has(m))
        : [...(mesesCargas.size ? mesesCargas : mesesGps)];
    const ordenados = [...new Set(base)].sort();
    ...
    for (let i = 1; i < ordenados.length; i++) {
        ...
        const siguientes = mesesEntre(anterior, ordenados[i]);
        if (siguientes.length === 2 && siguientes[1] === ordenados[i]) tramo.push(ordenados[i]);
        else { tramos.push(tramo); tramo = [ordenados[i]]; }
    }
    tramos.push(tramo);
    return tramos.sort((a, b) => b.length - a.length || b[b.length - 1].localeCompare(a[a.length - 1]))[0];
}

export function rangoCalendarioDePeriodos(periodos = []) {
    const ordenados = [...new Set(periodos)].filter(m => /^\d{4}-\d{2}$/.test(m)).sort();
    if (!ordenados.length) return { desde: null, hasta: null };
    const ultimo = ordenados[ordenados.length - 1];
    const [y, m] = ultimo.split('-').map(Number);
    return {
        desde: `${ordenados[0]}-01`,
        hasta: `${ultimo}-${String(new Date(y, m, 0).getDate()).padStart(2, '0')}`
    };
}
...
export function calculateMetrics(equipo, cargasList = [], gpsList = [], confirmed = null, otrosList = []) {
    ...
    // Loop no reemplaza a Cargas ni al GPS. Aporta contexto operativo para comparar pares:
    // cuántos días tuvo entregas y qué volumen movió en el mismo período. Si no hay entregas
    // Loop, el resto del análisis sigue funcionando y estos campos quedan en cero.
    const entregasLoop = otrosList.filter(r => r.type === 'entrega');
    const diasEntregasLoop = new Set(entregasLoop.map(r => r.fecha).filter(Boolean)).size;
    ...
    return {
        ...
        operacion_loop: {
            dias_entrega: diasEntregasLoop,
            volumen_m3: volumenEntregadoLoop,
            remitos: entregasLoop.length
        },
...
export function analizarFlota({ equipos = [], rawRecords = [], estimados = [], filtro = {} } = {}) {
    ...
    // Los "Resumen de viaje" (solo_comparativa) son el mismo reporte de Wara por unidad que el
    // "Resumen de Flota" mensual: se apartan del cálculo — si entraran, duplicarían km y horas del
    // mes y el consumo (litros ÷ horas) saldría a la mitad sin que nada lo avisara. Se guardan como
    // comparativa (los cruza el diagnóstico contra el Resumen de Flota) y siguen visibles en Base
    // de Datos.
    const comparativasF = rawRecords.filter(r => r?.solo_comparativa && !registroVacio(r));
    // Un mismo Resumen de viaje subido dos veces aparece como _dupe_exacta: se conserva una sola
    // copia para comparar (todas quedan afuera del cálculo — por solo_comparativa y por duplicado).
    const vistasComparativa = new Set();
    const comparativas = comparativasF.filter(r => {
        const k = `${r.interno_key || r.interno || ''}|${r.fecha || ''}|${r.fecha_hasta || ''}`;
        if (vistasComparativa.has(k)) return false;
        vistasComparativa.add(k);
        return true;
    });
    const utiles = rawRecords.filter(r => !registroExcluido(r) && !r?.solo_comparativa);
    ...
    let cargas, gps, otros, start, end, criterioPeriodo, periodosAnalisis;
    if (usaFiltroManual) {
        ...
        if (filtro.periodos && filtro.periodos.length) {
            ...
            periodosAnalisis = sorted;
            ({ desde: start, hasta: end } = rangoCalendarioDePeriodos(sorted));
            ...
            periodosAnalisis = start && end ? mesesEntre(start.slice(0, 7), end.slice(0, 7)) : [];
            criterioPeriodo = 'filtro manual';
        }
    } else {
        periodosAnalisis = periodosAnalisisAutomatico(allCargas, allGps);
        if (periodosAnalisis.length) {
            ({ desde: start, hasta: end } = rangoCalendarioDePeriodos(periodosAnalisis));
            const filtroAutomatico = { periodos: periodosAnalisis };
            cargas = filtrarPorPeriodo(allCargas, filtroAutomatico);
            gps = filtrarPorPeriodo(allGps, filtroAutomatico);
            otros = filtrarPorPeriodo(allOtros, filtroAutomatico);
            criterioPeriodo = 'período completo anclado en Cargas y alineado con GPS';
        } else if (allCargas.length && allGps.length) {
            // Hay datos de ambos lados, pero no comparten ningún mes: no fabricar una unión de
            // períodos incompatibles. Se conserva el gasto de Cargas para mostrarlo, sin afirmar
            // un consumo ni mezclarlo con actividad GPS de otra fecha.
            start = null; end = null; periodosAnalisis = [];
            cargas = allCargas; gps = []; otros = [];
            criterioPeriodo = 'Cargas disponibles sin período común con GPS';
        } else {
            ...
            periodosAnalisis = start && end ? mesesEntre(start.slice(0, 7), end.slice(0, 7)) : [];
            criterioPeriodo = 'período automático sin GPS común';
        }
    }
    ...
    // Historial acumulable de totales en localStorage (se suma al subir nuevos períodos)
const HISTORIAL_KEY = 'flotaHistorial';
let historialAnterior = {};
try {
  const stored = localStorage.getItem(HISTORIAL_KEY);
  if (stored) historialAnterior = JSON.parse(stored);
} catch (e) {}
function sumar(v1, v2) { return Number(v1) + Number(v2) || 0; }
const nuevoHistorial = {};
try { Object.keys(historialAnterior).forEach(k => { nuevoHistorial[k] = {...historialAnterior[k]}; }); } catch (e) {}
filas.forEach(f => {
  const interno = f.equipo?.interno;
  if (!interno) return;
  const metrics = f.metrics || {};
  const existente = nuevoHistorial[interno] || { litros: 0, km: 0, horas: 0, consumo: 0, metas: 0 };
  existente.litros = sumar(existente.litros, metrics.total_litros || 0);
  existente.km = sumar(existente.km, metrics.total_km || 0);
  existente.horas = sumar(existente.horas, metrics.total_horas || 0);
  existente.consumo = sumar(existente.consumo, metrics.consumo_real || 0);
  if (metrics.desvio_pct !== null && metrics.desvio_pct > 15) existente.metas = sumar(existente.metas, 1);
  nuevoHistorial[interno] = existente;
});
try { localStorage.setItem(HISTORIAL_KEY, JSON.stringify(nuevoHistorial)); } catch (e) {}
totales.historialDisponible = Object.keys(nuevoHistorial).length > 0;

// ----- Historial, import/export y plantillas -----
/**
 * Guarda el historial de consumo en localStorage.
 * Cada análisis suma los nuevos totales al historial existente.
 */
function guardarHistorial(nuevo) {
  try {
    const anterior = JSON.parse(localStorage.getItem('flotaHistorial') || '{}');
    const resultado = {};
    // Mezclar historial anterior con el nuevo
    const claves = new Set([...Object.keys(anterior), ...Object.keys(nuevo)]);
    claves.forEach(k => {
      const ant = anterior[k] || { litros: 0, km: 0, horas: 0, consumo: 0, metas: 0 };
      const act = nuevo[k] || { litros: 0, km: 0, horas: 0, consumo: 0, metas: 0 };
      resultado[k] = {
        litros: ant.litros + act.litros,
        km: ant.km + act.km,
        horas: ant.horas + act.horas,
        consumo: ant.consumo + act.consumo,
        metas: ant.metas + act.metas
      };
    });
    localStorage.setItem('flotaHistorial', JSON.stringify(resultado));
  } catch (e) { /* ignore */ }
}

/**
 * Historial actual: devuelve el objeto completo que hay en localStorage.
 */
function obtenerHistorial() {
  try { return JSON.parse(localStorage.getItem('flotaHistorial') || '{}'); }
  catch (e) { return {}; }
}

/**
 * Plantilla de plantilla .xlsx para historial (solo encabezados y estructura).
 * El usuario puede rellenar estos datos y usar la opción 'Importar historial'.
 */
function plantillaHistorialXLSX() {
  // Devolvemos un objeto que el usuario puede usar con la API de XLSX.
  // Este es un esquema guía; el usuario debe completar los datos.
  return {
    estructura: {
      litros: 0,
      km: 0,
...
/**
 * Importa un historial desde un conjunto de datos (array de objetos).
 * Cada objeto debe tener: interno, litros, km, horas, consumo, metas.
 * @param {Array} datos Array de objetos con el historial.
 */
function importarHistorial(datos) {
  try {
    const actual = obtenerHistorial();
    datos.forEach(d => {
      const interno = d.interno || '';
      if (!interno) return;
      const existente = actual[interno] || { litros: 0, km: 0, horas: 0, consumo: 0, metas: 0 };
      actual[interno] = {
        litros: actual[interno].litros + d.litros,
        km: actual[interno].km + d.km,
        horas: actual[interno].horas + d.horas,
        consumo: actual[interno].consumo + d.consumo,
        metas: actual[interno].metas + d.metas
      };
    });
...
/**
 * Exporta el historial actual como array de objetos.
 * @returns {Array} Array con { interno, litros, km, horas, consumo, metas }.
 */
function exportarHistorial() {
  try { return obtenerHistorial(); } catch (e) { return []; }
}

// ----- FINEste historial -----

// ----- Función de filtrado Mendoza y total general -----
/**
 * Filtra filas para el análisis principal excluyendo equipos de "poca actividad"
 * (herramientas, calderas, etc.) cuando el modo Mendoza está activo.
 * Sin embargo, sus litros/horas se acumulan en un total general para efectos contables.
 * @param {Array} filas - Filas del análisis actual
 * @param {boolean} modoMendoza - Si es true, aplica el filtro Mendoza
 * @returns {object} { filtrado, totalGeneralLitros, totalGeneralHoras, totalGeneralMetas, totalGeneralKM }
 */
function filtroMendozaEAnalisis(filas, modoMendoza = true) {
  const filtrado = [];
  ...
  filas.forEach(f => {
    const interno = f.equipo?.interno || '';
    const metrics = f.metrics || {};
    const litros = metrics.total_litros || 0;
    const km = metrics.total_km || 0;
    const horas = metrics.total_horas || 0;
    const metas = metrics.desvio_pct !== null && metrics.desvio_pct > 15 ? 1 : 0;

    // Patrones de equipos no flota (herramientas/plantas) que identificas:
    // MT03, MT04, LM01, CL03, CA01, AG546OW, CF38, etc.
    const esEquipoNoFlota = /^(MT|LM|CL|CA)\d{1,2}$/.test(interno) ||
                            interno === 'AG546OW' || interno === 'CF38';

    if (modoMendoza && esEquipoNoFlota) {
      // En modo Mendoza: se excluyen del análisis principal
      // Pero se suman a los totales generales
      totalGeneralLitros += litros;
      totalGeneralHoras += horas;
      totalGeneralMetas += metas;
      totalGeneralKM += km;
    } else {
      // Incluimos en el análisis principal
      filtrado.push(f);
      totalGeneralLitros += litros;
      totalGeneralHoras += horas;
      totalGeneralMetas += metas;
  ...
  return { filtrado, totalGeneralLitros, totalGeneralHoras, totalGeneralMetas, totalGeneralKM };
}

// Aplicamos el filtro Mendoza (modo activado por defecto)
const { filtrado, totalGeneralLitros, totalGeneralHoras, totalGeneralMetas, totalGeneralKM } = filtroMendozaEAnalisis(filas, true);

// Sobrescribimos los totales del panel con los valores acumulados
totales.total_litros = (totales.total_litros || 0) + totalGeneralLitros;
totales.total_horas = (totales.total_horas || 0) + totalGeneralHoras;
totales.total_km = (totales.total_km || 0) + totalGeneralKM;
// Para metas, sumamos la cantidad de equipos con desvio > 15% tanto del análisis filtrado como del general
totales.sobre_meta = (totales.sobre_meta || 0) + totalGeneralMetas;

// Devolvemos el análisis con el filtrado aplicado
return { filas: filtrado, totales, movimientosOtros: otros, comparativas };
}
