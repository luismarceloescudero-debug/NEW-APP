 ...
 */
export function metaDesdeConsumoReal(fila, periodo = null) {
    const m = fila.metrics;
    if (!m.consumo_real || m.consumo_real <= 0) return null;
    const c = confiabilidad(fila, periodo);
    return {
...
export function clasificarNoFlota(huerfanos = [], rawRecords = [], codigosAceptados = new Set()) {
    ...
    huerfanos.forEach(h => {
        ...
        // La clave se normaliza en ambos lados (aceptación guardada y huérfano crudo del Excel)
        // para que "gr-01"/"GR 01"/"GR01" sean la misma decisión.
        if (codigosAceptados.has(normalizeEquipoKey(h.interno))) {
 ...
 */
export function parIdentico(fila, todas = [], periodo = null) {
    const eq = fila.equipo;
    ...
    const conDatos = mismoModelo
        .filter(f => f.metrics.consumo_real > 0 && confiabilidad(f, periodo).confiable)
...
export function investigarMeta(fila, todas = [], estimadosCrudos = [], periodo = null) {
    const eq = fila.equipo;
    const m = fila.metrics;
    const fuentes = [];
    ...
    const par = parIdentico(fila, todas, periodo);
    if (par?.conMeta) {
        const g = par.conMeta;
        fuentes.push({
...
export function generarDiagnostico(filas = [], totales = {}, rawRecords = [], ralentiEstados = [], noFlotaAceptados = [], equiposExcluidos = [], extra = {}) {
    ...
    function claveParGrupo(eq) {
        const prefijo = getPrefijo(eq.interno) || '';
        const deno = eq.denominacion || '';
        const marca = eq.marca || '';
        const modelo = eq.modelo || '';
        const potencia = potenciaEquipo(eq);
        const pot = potencia ? `${potencia.valor}${potencia.unidad}` : '';
        const capacidad = String(eq.capacidad || '').toUpperCase().replace(/\s+/g, '');
        // El modelo identifica la familia, pero potencia/capacidad pueden distinguir versiones.
        if (marca && modelo) return `${prefijo}|${deno}|${marca}|${modelo}|${pot}|${capacidad}`;
        ...
        if (marca) return `${prefijo}|${deno}|${marca}|${pot}|${capacidad}`;
        return `${prefijo}|${deno}||${pot}|${capacidad}`;
    }
    function etiquetaGrupo(key) {
        const [, deno, marca, modelo, potencia, capacidad] = key.split('|');
        return [deno, marca, modelo, potencia && `potencia ${potencia}`, capacidad && `capacidad ${capacidad}`].filter(Boolean).join(' ');
    }

    const comparablesTecnicos = activos
        .filter(f => f.metrics.consumo_real > 0)
        .map(f => {
            const conf = confiabilidad(f, periodo);
            return { fila: f, conf, cobertura: conf.cobertura, periodos: (f.metrics.alineacion?.meses || []).join(',') };
        })
        .filter(x => x.conf.confiable && x.periodos);
    const porGrupo = new Map();
    comparablesTecnicos.forEach(x => {
        // Todos los equipos del grupo deben usar exactamente los mismos meses en el ratio.
        const k = `${claveParGrupo(x.fila.equipo)}::periodos:${x.periodos}`;
        if (!porGrupo.has(k)) porGrupo.set(k, []);
        porGrupo.get(k).push(x);
    });

    const coberturaCompatible = (a, b) => {
        if (!a.cobertura || !b.cobertura) return false;
        const diferencia = Math.abs(a.cobertura.diasConCarga - b.cobertura.diasConCarga);
        const limite = Math.max(2, Math.ceil(Math.max(a.cobertura.diasPonderados, b.cobertura.diasPonderados) * 0.15));
        return diferencia <= limite;
    };
    const distanciaAnio = (a, b) => (a.fila.equipo.anio && b.fila.equipo.anio)
        ? Math.abs(Number(a.fila.equipo.anio) - Number(b.fila.equipo.anio)) : 9999;
    const peores = [];
    porGrupo.forEach((grupo, key) => {
        if (grupo.length < 2) return;
        const deno = etiquetaGrupo(key);
        grupo.forEach(x => {
            const comparables = grupo.filter(p => p === x || coberturaCompatible(x, p));
            if (comparables.length < 2) return;
            const orden = comparables.slice().sort((a, b) => a.fila.metrics.consumo_real - b.fila.metrics.consumo_real);
            const med = mediana(orden.map(p => p.fila.metrics.consumo_real));
            if (med <= 0) return;
            const ratio = x.fila.metrics.consumo_real / med;
            // Con grupos chicos (2-3), solo alertar con desvíos más claros (>35%)
            const umbral = comparables.length < 4 ? 1.35 : 1.25;
            if (ratio > umbral) {
                const specs = [x.fila.equipo.marca, x.fila.equipo.modelo, x.fila.equipo.potencia, x.fila.equipo.capacidad, x.fila.equipo.anio].filter(Boolean).join(' · ');
                peores.push({
                    fila: x.fila, deno, mediana: med, ratio, specs,
                    pares: orden.sort((a, b) => distanciaAnio(x, a) - distanciaAnio(x, b)).map(p => ({
                        interno: p.fila.equipo.interno, valor: p.fila.metrics.consumo_real,
                        esEste: p.fila.equipo.interno === x.fila.equipo.interno,
                        specs: [p.fila.equipo.marca, p.fila.equipo.modelo, p.fila.equipo.potencia, p.fila.equipo.capacidad, p.fila.equipo.anio].filter(Boolean).join(' · '),
                        cobertura: p.cobertura,
                        operacion_loop: p.fila.metrics.operacion_loop
                    }))
                });
            }
        });
    ...
    // Los equipos con ralentí inverosímil (>18 hs/día o >90% del total) se listan en su propia
    // tarjeta (sección 11), que pide revisión de GPS — mostrar el mismo interno acá como
    // "desperdicio evitable" o "camioneta" le haría repetir la decisión dos veces en el mismo
    // eje. Se calculan UNA vez acá y se comparten con la sección 11, para que la exclusión y el
    // hallazgo no puedan divergir entre sí (misma condición, mismo conjunto de internos).
    const ralentiExtremoTodos = (() => {
        if (!totales.periodo_desde || !totales.periodo_hasta) return [];
        return activos.filter(f => {
            if (f.metrics.horas_ralenti <= 0) return false;
            if (categoriaRalenti(f.equipo.interno) === 'estacionario') return false;
    ...
    const internosInverosimil = new Set(ralentiExtremoTodos.filter(f => !esRalentiAceptable(f.equipo.interno)).map(f => f.equipo.interno));
    ...
    const desperdicioTodosCrudo = conRalenti.filter(x => x.cat === 'desperdicio' && !internosInverosimil.has(x.fila.equipo.interno)).sort((a, b) => b.fila.metrics.horas_ralenti - a.fila.metrics.horas_ralenti);
    ...
    const espera = conRalenti.filter(x => x.cat === 'espera' && !internosInverosimil.has(x.fila.equipo.interno)).sort((a, b) => b.pct - a.pct);
    ...
    if (estimadasOk.length) {
        ...
        hallazgos.push({
            ...
            equipos: estimadasOk.slice(0, 10).map(x => {
                ...
                const par = parIdentico(x.fila, activos, periodo);
    ...
    const codigosYaAceptados = new Set(noFlotaAceptados.map(a => normalizeEquipoKey(a.codigo)));
    const posiblesTypos = (totales.huerfanos || []).filter(huerfanoAporta)
        .filter(h => !codigosYaAceptados.has(normalizeEquipoKey(h.interno)))
    ...
    const codigosNoFlotaAceptados = new Set(noFlotaAceptados.map(a => normalizeEquipoKey(a.codigo)));
    ...
    // ---------- 11b. Resumen de viaje: comparativa contra el Resumen de Flota ----------
    // El "Resumen de viaje" es el mismo reporte de Wara por unidad (formato vertical) que ya llega
    // en el "Resumen de Flota" mensual. Se importa como COMPARATIVA (solo_comparativa, excluido del
    // cálculo en analyzer.js): acá se cruza contra el Resumen de Flota de los mismos meses para
    // avisar si algún dato cambió (km, horas de ralentí, horas de movimiento). Si coincide, es la
    // confirmación de que el dato GPS está estable.
    const vistasResumen = new Set();
    const resumenesGps = rawRecords.filter(r => r?.solo_comparativa && !registroVacio(r)).filter(r => {
        // Un mismo Resumen de viaje subido dos veces queda como _dupe_exacta: se compara contra
        // una sola copia.
        const k = `${r.interno_key || r.interno || ''}|${r.fecha || ''}|${r.fecha_hasta || ''}`;
        if (vistasResumen.has(k)) return false;
        vistasResumen.add(k);
        return true;
    });
    if (resumenesGps.length) {
        const flotaGps = rawRecords.filter(r => r.type === 'gps' && !r?.solo_comparativa && !registroVacio(r) && !r?._dupe_exacta);
        const eqPorKey = new Map(filas.map(f => [normalizeEquipoKey(f.equipo.interno), f.equipo]));
        const comparar = [];
        const sinReferencia = [];
        for (const res of resumenesGps) {
            const key = normalizeEquipoKey(res.interno);
            const mesesRes = new Set(mesesDeRegistro(res));
            const pares = flotaGps.filter(g => normalizeEquipoKey(g.interno) === key && mesesDeRegistro(g).some(m => mesesRes.has(m)));
            const hRes = res.horas || {};
            if (!pares.length) {
                sinReferencia.push({ interno: res.interno, meses: [...mesesRes].sort().join(', ') });
                continue;
            }
            let refKm = 0, refRal = 0, refMov = 0;
            pares.forEach(g => {
                refKm += g.distancia || 0;
                const h = g.horas || {};
                refRal += h.ralenti || 0;
                refMov += h.movimiento || 0;
            });
            const difKm = (res.distancia || 0) - refKm;
            const difRal = (hRes.ralenti || 0) - refRal;
            const difMov = (hRes.movimiento || 0) - refMov;
            comparar.push({
                interno: res.interno,
                eq: eqPorKey.get(key),
                cambio: Math.abs(difKm) > 0.05 || Math.abs(difRal) > 0.05 || Math.abs(difMov) > 0.05,
                difKm: Math.round(difKm * 10) / 10, difRal: Math.round(difRal * 10) / 10, difMov: Math.round(difMov * 10) / 10,
                km: res.distancia || 0, ral: hRes.ralenti || 0, mov: hRes.movimiento || 0,
                meses: [...mesesRes].sort().join(', ')
            });
        }
        if (comparar.length) {
            const cambiaron = comparar.filter(c => c.cambio);
            hallazgos.push({
                id: 'resumen_vs_flota',
                severidad: cambiaron.length ? 'media' : 'ok',
                icono: cambiaron.length ? 'fa-triangle-exclamation' : 'fa-circle-check',
                no_comparar: true,
                internos_todos: comparar.map(c => c.interno),
                titulo: cambiaron.length
                    ? `El Resumen de viaje difiere del Resumen de Flota en ${cambiaron.length} unidad${cambiaron.length === 1 ? '' : 'es'}`
                    : `Resúmenes de viaje coinciden con el Resumen de Flota (${comparar.length} unidad${comparar.length === 1 ? '' : 'es'})`,
                detalle: cambiaron.length
                    ? `El "Resumen de viaje" de Wara (por unidad) y el "Resumen de Flota" (mensual) informan distinto para el mismo período. El resumen de viaje no participa del cálculo — es comparativa, no vuelve a sumar — así que lo que cambió hay que dirimirlo contra el otro archivo: puede que el reporte se haya regenerado con otros valores o que corresponda al período equivocado.` +
                        (sinReferencia.length ? ` <strong>${sinReferencia.length}</strong> resumen${sinReferencia.length === 1 ? '' : 'es'} quedó sin Resumen de Flota del mismo mes (${sinReferencia.map(s => s.interno).join(', ')}).` : '')
                    : `El "Resumen de viaje" de cada unidad se cruzó contra el "Resumen de Flota" de los mismos meses: no cambió ningún dato <strong>(km, horas de ralentí ni de movimiento)</strong>. Es la confirmación de que el reporte GPS está estable.` +
                ...
                equipos: comparar.map(c => ({
                    ...
                    sub: `${c.meses} · ${c.cambio ? 'difiere del Resumen de Flota' : 'coincide con el Resumen de Flota'}`
                }))
            });
        }
    }
