...
import { getPrefijo } from '../data/normalizer.js';
...
let overlay = null;
let periodoRef = null;  // {desde, hasta, periodos} del análisis activo — denominador de las tasas
...
const METRICAS = [
    ...
    { key: 'dias_entrega_loop', label: 'Días con entregas Loop', unidad: 'días', dec: 0, get: f => f.metrics.operacion_loop?.dias_entrega || 0, mejorEsMenor: null },
    { key: 'volumen_loop', label: 'Volumen entregado Loop', unidad: 'm³', dec: 1, get: f => f.metrics.operacion_loop?.volumen_m3 || 0, mejorEsMenor: null },
...
const METRICAS_NORMALIZADAS = [
    ...
    { key: 'volumen_dia_loop', esTasa: true, label: 'm³ Loop por día con entrega', unidad: 'm³', dec: 1, mejorEsMenor: null,
      get: (f, c) => c.loop?.dias_entrega > 0 ? c.loop.volumen_m3 / c.loop.dias_entrega : null },
...
function contextoDe(fila) {
    ...
    const meses = periodoRef?.periodos?.length ? periodoRef.periodos : (cobMes?.listaMeses || []);

    // El denominador es el mismo período para todos los equipos. Así 19/22 días de carga se
    // compara contra 22/22, sin convertir la menor actividad en una ventaja matemática.
    ...
    return {
        ...
        mesesPeriodo: periodoRef?.periodos?.length || cobMes?.mesesPeriodo || 0,
        listaMeses: meses,
        diasConCarga: cob?.diasConCarga || 0,
        diasHabiles: cob?.diasPonderados || 0,
        coberturaPct: cob?.pct ?? null,
        loop: fila.metrics.operacion_loop || { dias_entrega: 0, volumen_m3: 0, remitos: 0 },
...
export function abrirComparativa(analisis, internosIniciales = []) {
    ...
    periodoRef = analisis?.totales?.periodo_desde && analisis?.totales?.periodo_hasta
        ? { desde: analisis.totales.periodo_desde, hasta: analisis.totales.periodo_hasta, periodos: analisis.totales.periodos_analisis || [] }
        : null;
    modo = 'totales';
    ...
    const agregarDesdeInput = () => {
        ...
        const fila = filasRef.find(f =>
            ...
            (f.equipo.marca || '').toUpperCase().includes(val) ||
            (f.equipo.modelo || '').toUpperCase().includes(val)
        );
        if (!fila) {
...
function pintar() {
    ...
    chips.innerHTML = filas.length ? filas.map(f => `
        <span class="comparar-chip">
            ${esc(f.equipo.interno)}<small>${esc([f.equipo.denominacion, f.equipo.marca, f.equipo.modelo].filter(Boolean).join(' · '))}</small>
    ...
    const coberturaDistinta = new Set(ctxs.map(c => c.diasConCarga)).size > 1;
    const filaCobertura = `<tr class="cmp-fila-periodo">
        <td class="cell-key">Días con carga / hábiles${coberturaDistinta ? ' <b>(desiguales)</b>' : ''}</td>
        ${ctxs.map(c => `<td class="${coberturaDistinta ? 'cmp-worst' : ''}" title="Mismo período: ${esc(c.listaMeses.join(', '))}">${nf(c.diasConCarga)} de ${nf(c.diasHabiles, 1)} <small>(${c.coberturaPct === null ? 'sin dato' : `${nf(c.coberturaPct)}%`})</small></td>`).join('')}
    </tr>`;
    ...
    tabla.innerHTML = `
        <thead>
            <tr>
                <th>Métrica</th>
                ${filas.map(f => `<th>${esc(f.equipo.interno)}<div class="th-sub">${esc(f.equipo.dominio || f.equipo.denominacion || '')}</div><small>${esc([f.equipo.marca, f.equipo.modelo, f.equipo.potencia, f.equipo.capacidad, f.equipo.anio].filter(Boolean).join(' · '))}</small></th>`).join('')}
            </tr>
        </thead>
        <tbody>
            ${filaPeriodo}
...
function validarCriterios(filas) {
    ...
    // La primera comparación válida es entre la misma familia operativa: MX con MX, CM con CM.
    // Se permite continuar para explorar, pero se deja visible que el resultado no es un benchmark.
    const familias = [...new Set(filas.map(f => getPrefijo(f.equipo.interno)).filter(Boolean))];
    if (familias.length > 1) {
        avisos.push({
            tipo: 'familia', icono: 'fa-people-arrows', color: 'aviso-alta',
            texto: `Se mezclan familias (${familias.join(' y ')}): primero compará equipos del mismo tipo operativo.`
        });
    }

    const ctxs = filas.map(contextoDe);
    const diasCarga = ctxs.map(c => c.diasConCarga);
    if (diasCarga.every(Boolean) && Math.max(...diasCarga) - Math.min(...diasCarga) > Math.max(2, Math.ceil(Math.max(...ctxs.map(c => c.diasHabiles)) * 0.15))) {
        avisos.push({
            tipo: 'cobertura', icono: 'fa-calendar-check', color: 'aviso-media',
            texto: `Días con carga desiguales (${diasCarga.join(' vs ')}): el consumo debe leerse junto a la cobertura del período, no como si todos hubieran trabajado la misma cantidad de días.`
        });
    }

    const loopDias = ctxs.map(c => c.loop?.dias_entrega || 0);
    if (loopDias.some(Boolean) && loopDias.some(d => !d)) {
        avisos.push({
            tipo: 'loop', icono: 'fa-truck-ramp-box', color: 'aviso-media',
            texto: 'La información de entregas Loop no está completa para todos los equipos seleccionados; el volumen y los días de entrega se muestran como contexto, no como dato comparable para los que no tienen registro.'
        });
    }
