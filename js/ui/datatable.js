...
async function renderMaestro() {
    ...
    document.getElementById('table-header').innerHTML =
        ...
        CAMPOS_MAESTRO.map(c => `<th${c.num ? ' class="th-num"' : ''}>${esc(c.label)}</th>`).join('') +
        '<th class="th-num" title="Calculado: litros/hora o litros/100km realmente medidos en el período vigente">Consumo real</th>' +
        '<th class="th-num" title="Cantidad de cargas que respaldan el Consumo real de esa fila">Cargas</th>' +
        '<th class="th-num" title="Diferencia entre el Consumo real y la Meta">vs Meta</th>' +
    ...
    document.getElementById('table-body').innerHTML = pagina.map(e => {
        ...
        return `<tr data-interno="${esc(e.interno)}" class="${tieneAsociado} ${sel ? 'row-sel' : ''}" ${e.equipo_asociado ? `title="Asociado a ${esc(e.equipo_asociado)}"` : ''}>
            ...
            ${CAMPOS_MAESTRO.map(c => {
                ...
                return `<td class="cell-edit${c.num ? ' cell-num' : ''}${marca}" contenteditable="true" data-campo="${c.k}" data-num="${c.num ? 1 : 0}">${esc(v)}</td>`;
            }).join('')}
...
async function renderEstimados() {
    ...
    document.getElementById('table-header').innerHTML =
        ...
        '<th class="th-num" title="Lo que vino escrito en la planilla Consumos Estimados">Estimado (planilla)</th><th>Unidad</th>' +
        '<th class="th-num" title="La meta que efectivamente rige hoy en el maestro">Meta actual (maestro)</th>' +
        '<th class="th-num" title="Diferencia entre el Estimado de la planilla y la Meta actual">vs Meta</th>' +
        '<th class="th-num" title="Litros/hora o litros/100km medidos de verdad en el período vigente del Panel — no lo que dice la planilla, lo que consumió">Consumo real</th>' +
        '<th class="th-num" title="Diferencia entre el Consumo real medido y la Meta actual del maestro">vs real</th>' +
        '<th class="th-num">Cargas</th><th class="th-num">GPS</th><th>Estado</th>';
...
async function renderConsumoReal() {
    ...
    document.getElementById('table-header').innerHTML =
        '<th class="th-sel"><input type="checkbox" id="th-sel-all" title="Seleccionar todos"></th>' +
        '<th>Interno</th><th>Dominio</th><th>Denominación</th>' +
        '<th class="th-num">Consumo real</th><th class="th-num">Cargas</th>' +
        `<th title="Sí/No: ${MIN_CARGAS_TXT} cargas y ${COBERTURA_TXT}% de cobertura mínima de días hábiles">Confiable</th>` +
        '<th class="th-num" title="Proporción real detrás del veredicto: días con carga sobre días hábiles del período">Cobertura</th>' +
        '<th class="th-num">Meta actual</th><th class="th-num">vs Meta</th><th>Acciones</th>';
...
async function renderMovimientos(tipo) {
     ...
     *
     * Además, los códigos marcados como "así está bien" en el Panel (consumo fuera de la flota:
     * vehículos de préstamo/demo, planta…) tampoco son un error acá: la aceptación es global
     * (no se pierde al cambiar de período). La tabla tenía su propio criterio de huérfano que
     * ignoraba esa decisión, por eso el mismo dominio volvía a aparecer con "Sin asignar"
     * aunque se lo hubiera aceptado.
     */
    const noflAceptados = await getNoFlotaAceptados();
    const noflAceptadosSet = new Set(noflAceptados.map(a => normalizeEquipoKey(a.codigo)));
    const esNoFlotaAceptada = (r) => noflAceptadosSet.has(normalizeEquipoKey(r.interno_key || r.interno || r.dominio_key || r.dominio || ''));
    const esHuerfanaDe = (r) => !resolverEquipo(r, idxMaestro) && !esNoFlotaAceptada(r);
    ...
    document.getElementById('table-header').innerHTML =
        ...
        cols.map(c => `<th${c.num !== undefined || c.money ? ' class="th-num"' : ''}>${esc(c.label)}${noEditable.has(c.k) ? '' : `
    ...
    document.getElementById('table-body').innerHTML = pagina.map(r => {
        ...
        return `<tr${rowClass ? ` class="${rowClass}"` : ''}${dataAttrs}>
            ...
            ${cols.map(c => {
                ...
                if (noEditable.has(c.k)) return `<td${c.num !== undefined || c.money ? ' class="cell-num"' : ''}>${v}</td>`;
                ...
                return `<td class="cell-edit${c.num !== undefined || c.money ? ' cell-num' : ''}" contenteditable="true" data-recid="${r.id}" data-campo="${esc(c.k)}" data-num="${c.num !== undefined ? 1 : 0}" data-label="${esc(c.label)}">${v}</td>`;
            }).join('')}
            ${accionTd}
        </tr>`;
