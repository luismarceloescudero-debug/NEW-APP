...
// excluirlo del análisis — acá se reutiliza desde las tarjetas de hallazgos (ralentí, sin
// actividad, estimación no creíble) para no tener un mecanismo aparte por cada uno.
...
const ACCIONES_PROPUESTAS = {
    ...
    datos_parciales: [
        { texto: 'Revisar y decidir equipo por equipo', icono: 'fa-list-check', accion: 'revisar_decidir' },
        { texto: 'Declarar km/horas estimados', icono: 'fa-gauge-high', accion: 'declarar_actividad' },
        { texto: 'Cómo corregir esto', icono: 'fa-lightbulb', accion: 'consejos' }
    ],
    bajo_uso: [
...
window.abrirActividadEstimada = (internos) => abrirActividadEstimada(internos, ultimoAnalisis);
        ...
        // El ralentí sí se acota al período: "este mes el ralentí de tal equipo se aceptó".
        // Los códigos de "consumo fuera de la flota" marcados como "así está bien" son una
        // decisión permanente (un vehículo de préstamo/demo o el consumo de planta no dejan de
        // serlo al cambiar de mes), así que NO se filtran por período: se desmarcan a mano
...
function poblarFiltrosPeriodo(rawRecords) {
    ...
    // Cargas ancla el período automático; GPS solo aporta la actividad que permite medirlo.
    // La selección se limita al tramo consecutivo común más largo para no presentar meses
    // separados como si fueran una serie continua. Una selección manual sigue teniendo prioridad.
    const periodosAutomaticos = periodosAnalisisAutomatico(
        rawRecords.filter(r => r.type === 'carga'),
        rawRecords.filter(r => r.type === 'gps')
    );
    if (view.meses.size === 0) periodosAutomaticos.forEach(ym => view.meses.add(ym));

    renderMesesGrid(periodos, mesesCargas, mesesGps, periodosAutomaticos);
}

/** Grilla interactiva de meses: cada botón muestra disponibilidad de cargas/GPS y se puede tildar. */
function renderMesesGrid(todosLosPeriodos, mesesCargas, mesesGps, periodosAutomaticos = []) {
    ...
    const periodosReales = [...new Set(periodosAutomaticos.length ? periodosAutomaticos : todosLosPeriodos)].sort();
    ...
    grid.querySelector('#meses-solo-cruzados')?.addEventListener('click', () => {
        view.meses.clear();
        periodosReales.forEach(ym => view.meses.add(ym));
        renderPanel();
    });
...
function renderDiagnostico(analisis, rawRecords = []) {
    ...
    const renderHallazgoCard = (h, i, esIgnorado) => {
        const abierto = diagAbiertos.has(h.id) ? diagAbiertos.get(h.id) : !esIgnorado;
        ...
        const accionesEnDialogo = esRalenti;
        ...
        <details class="diag-card ${sev[h.severidad]} ${esIgnorado ? 'diag-ignorado' : ''}${todoAtendido ? ' diag-todo-atendido' : ''}" data-id="${esc(h.id)}" ${abierto && !todoAtendido ? 'open' : ''}>
            ...
            <div class="diag-body">
                ...
                <div class="diag-acciones-bar">
                    ...
                    ${acciones.filter(a => !(ocultarResolverEIgnorar && a.accion === 'resolver') && !(h.id === 'datos_parciales' && a.accion === 'revisar_decidir') && !(accionesEnDialogo && ['detalle_ralenti', 'consejos'].includes(a.accion))).map(a => `<button class="btn-sm btn-diag-propuesta" data-accion="${esc(a.accion)}" data-hallazgo="${esc(h.id)}"><i class="fa-solid ${a.icono}"></i> ${esc(a.texto)}</button>`).join('')}
                    ${!accionesEnDialogo && h.equipos && h.equipos.length >= 2 && !h.no_comparar ? `<button class="btn-sm btn-diag-comparar-lista" data-hallazgo="${esc(h.id)}" title="Abrir comparativa con estos equipos"><i class="fa-solid fa-code-compare"></i> Comparar estos equipos</button>` : ''}
                    ...
                    ${!accionesEnDialogo && (esRalenti || h.id === 'sin_medicion') && h.equipos && h.equipos.length ? `<button class="btn-sm btn-ralenti-reclamo-lote" data-hallazgo="${esc(h.id)}" title="Genera un reclamo de revisión de GPS para cada equipo tildado en la lista de abajo"><i class="fa-solid fa-satellite-dish"></i> Reclamo GPS (selección)</button>` : ''}
                    ${!accionesEnDialogo && puedeReclamarGPS ? `<button class="btn-sm btn-ver-reclamos-gps" title="Ver los reclamos de revisión de GPS generados"><i class="fa-solid fa-list-check"></i> Reclamos GPS</button>` : ''}
                    ${!accionesEnDialogo && esRalenti && ralentiEstadosCache.some(r => r.estado === 'aceptable') ? `<button class="btn-sm btn-ver-ralenti-aceptados" title="Ver y desmarcar equipos con ralentí aceptable"><i class="fa-solid fa-list-check"></i> Ralentí aceptable (${ralentiEstadosCache.filter(r => r.estado === 'aceptable').length})</button>` : ''}
                    ...
                    ${esIgnorado ? `<button class="btn-sm btn-diag-restaurar" data-hallazgo="${esc(h.id)}" title="Volver a mostrar este hallazgo"><i class="fa-solid fa-eye"></i> Restaurar</button>` : ''}
                </div>
                        ...
                        <div class="comparativa-barras">
                            ${c.pares.map(p => `
                                <div class="cmp-fila ${p.esEste ? 'cmp-este' : ''}">
                                    ...
                                    <span class="cmp-val">${nf(p.valor, 2)}${p.cobertura ? `<small title="Días con carga sobre días hábiles del período"> · ${nf(p.cobertura.diasConCarga)}/${nf(p.cobertura.diasPonderados, 1)} días</small>` : ''}${p.operacion_loop?.dias_entrega ? `<small title="Contexto operativo de Loop"> · ${nf(p.operacion_loop.dias_entrega)} días Loop · ${nf(p.operacion_loop.volumen_m3, 1)} m³</small>` : ''}</span>
                                </div>`).join('')}
                        </div>
                    </div>`).join('') : ''}
                ${(() => {
 ...
 * alto" no se puede accionar del otro lado. Se arma con lo que la app ya calculó del Resumen de
 * Flota.
 */
        ...
        <div class="modal-overlay active" id="${modalId}">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p class="modal-note">Se guarda como registro interno en la app${esLote ? ', uno por equipo' : ''}. "Enviar por mail" abre tu cliente de correo con el mensaje ya redactado y dirigido a <strong>${SOPORTE_GPS}</strong>, incluyendo los datos medidos de cada equipo (horas reportadas, proporción de ralentí y kilómetros del Resumen de Flota).</p>
 ...
 */
function abrirResolucion(hallazgoId, analisis, rawRecords, internosIniciales = []) {
    const container = document.getElementById('modals-container');
    if (!container || !analisis) return;
    ...
    const internos = internosIniciales.length
        ? [...new Set(internosIniciales)].filter(i => porInterno.has(i) || (h.internos_todos || []).includes(i))
        : ((h.internos_todos && h.internos_todos.length) ? h.internos_todos : [...porInterno.keys()]);
...
function abrirInvestigacionMeta(interno) {
    ...
    const fila = ultimoAnalisis.filas.find(f => f.equipo.interno === interno);
    if (!fila) return;
    const inv = investigarMeta(fila, ultimoAnalisis.filas, datosCrudos?.estimados || [], periodoDeAnalisis(ultimoAnalisis));
...
function abrirRevisarDecidir(hallazgoId, analisis, rawRecords) {
    ...
    const equipos = listaInternos.map(interno => {
        ...
        const fila = (analisis.filas || []).find(f => f.equipo.interno === e.interno);
        return {
            ...e,
            cobertura: fila ? coberturaMensual(fila, {
                desde: analisis.totales.periodo_desde,
                hasta: analisis.totales.periodo_hasta
            }) : null
        };
    });
            ...
            <div class="modal-content modal-wide">
                <div class="modal-header">
                    ...
                    <div class="modal-header-actions">
                        <button class="btn-xs btn-rev-export" data-formato="xlsx" title="Exportar equipos, cobertura, estados y metas"><i class="fa-solid fa-file-excel"></i> Excel</button>
                        <button class="btn-xs btn-rev-export" data-formato="doc" title="Descargar un documento editable en Word"><i class="fa-solid fa-file-word"></i> Word</button>
                        <button class="btn-xs btn-rev-export" data-formato="pdf" title="Abrir una versión lista para imprimir o guardar como PDF"><i class="fa-solid fa-file-pdf"></i> PDF</button>
                        <button class="btn-close" data-close><i class="fa-solid fa-xmark"></i></button>
                    </div>
                </div>
                <div class="modal-body">
                    ...
                    <table class="data-table rev-tabla">
                        <thead><tr><th style="width:2rem"></th><th>Equipo</th><th>Dato</th><th>Período / cobertura</th><th>Detalle</th><th style="width:5rem"></th></tr></thead>
                        <tbody>
                            ${equipos.map(e => {
                                const at = atendidos.get(e.interno);
                                const cob = e.cobertura;
                                const cobertura = cob && cob.mesesPeriodo
                                    ? `${cob.conDatos} de ${cob.mesesPeriodo} meses${cob.listaMeses?.length ? `<br><small>${esc(cob.listaMeses.join(', '))}</small>` : ''}`
                                    : 'Sin detalle de meses';
                                return `<tr data-interno="${esc(e.interno)}"${at ? ' class="rev-atendido"' : ''}>
                                    <td><input type="checkbox" class="chk-rev" data-interno="${esc(e.interno)}"${at ? ' disabled' : ' checked'}></td>
                                    <td class="cell-key">${esc(e.interno)}<br><small>${esc(e.denominacion || '')}</small></td>
                                    <td class="cell-num">${esc(e.texto || '')}</td>
                                    <td class="rev-cobertura">${cobertura}</td>
                    ...
                    <div class="diag-acciones-bar" style="margin-top:.4rem">
                        ${btn('estado', 'fa-clipboard-list', 'Estado', 'Backup, taller, sin chofer, temporada baja… ajusta el denominador de cobertura')}
                        ${btn('declarar', 'fa-calculator', 'Declarar km/horas estimados', 'Cargar la actividad estimada para los equipos seleccionados')}
                        ${btn('investigar_meta', 'fa-magnifying-glass-chart', 'Investigar consumos estimados', 'Revisar las fuentes disponibles para la meta de los equipos seleccionados')}
                        ${btn('resolver', 'fa-wand-magic-sparkles', 'Cómo lo resuelvo', 'Mostrar el veredicto y la acción sugerida por equipo')}
                        ${btn('cargar_meta', 'fa-bullseye', 'Cargar metas para estimar actividad', 'Abrir la carga masiva de metas para habilitar la estimación de actividad')}
                        ${btn('consejos', 'fa-lightbulb', 'Cómo corregir esto', 'Abrir la guía de corrección de este hallazgo')}
                        ${btn('detalle', 'fa-chart-simple', 'Ver detalle de ralentí por equipo', 'Abrir el detalle del equipo seleccionado')}
                        ${btn('comparar', 'fa-code-compare', 'Comparar estos equipos', 'Abrir la comparativa con los equipos tildados')}
                        ${btn('ralenti', 'fa-check-double', 'Investigar y marcar aceptable (selección)', 'Revisar y sacar del hallazgo de ralentí a los equipos seleccionados')}
                        ${btn('reclamo', 'fa-satellite-dish', 'Reclamo GPS (selección)', 'Generar un pedido de revisión por cada equipo seleccionado')}
                        ${btn('reclamos', 'fa-list-check', 'Reclamos GPS', 'Ver todos los reclamos GPS abiertos y cerrados')}
                        ${btn('aceptados', 'fa-list-check', `Ralentí aceptable (${ralentiEstadosCache.filter(r => r.estado === 'aceptable').length})`, 'Ver y desmarcar equipos marcados como aceptables')}
                        <span class="diag-acciones-sep"></span>
    ...
    modal.querySelectorAll('[data-close]').forEach(b => b.addEventListener('click', cerrar));
    modal.addEventListener('click', (e) => { if (e.target === modal) cerrar(); });
    modal.querySelectorAll('.btn-rev-export').forEach(b => b.addEventListener('click', () => {
        exportarRevisionDecidir(b.dataset.formato, h, equipos, analisis, modal);
    }));

    const seleccion = () => [...modal.querySelectorAll('.chk-rev:checked:not(:disabled)')].map(c => c.dataset.interno);
    ...
    modal.querySelectorAll('.btn-rev-accion').forEach(b => {
        b.addEventListener('click', async () => {
            ...
            if (!internos.length && !['consejos', 'reclamos', 'aceptados', 'cargar_meta'].includes(b.dataset.accion)) {
                alert('No hay equipos tildados.'); return;
            }
            const per = periodoDeAnalisis(analisis);
            switch (b.dataset.accion) {
                case 'detalle':
                    if (internos.length !== 1) { alert('Para ver el detalle de ralentí, tildá un solo equipo.'); return; }
                    cerrar(); buscarEquipo(internos[0]); break;
                case 'consejos':
                    cerrar(); abrirConsejos(hallazgoId); break;
                case 'investigar_meta':
                    cerrar();
                    if (internos.length === 1) abrirInvestigacionMeta(internos[0]);
                    else abrirAjusteMetas(analisis, 'todos', internos);
                    break;
                case 'resolver':
                    cerrar(); abrirResolucion(hallazgoId, ultimoAnalisis, rawRecords, internos); break;
                case 'cargar_meta':
                    cerrar(); abrirAjusteMetas(analisis, 'sin_meta', internos); break;
                case 'reclamos':
                    cerrar(); abrirReclamosGPS(); break;
                case 'aceptados':
                    cerrar(); abrirRalentiAceptados(analisis, rawRecords); break;
                case 'sanjuan': {
                    if (!confirm(`¿Marcar ${internos.length} equipo${internos.length === 1 ? '' : 's'} como de SAN JUAN y apartarlo${internos.length === 1 ? '' : 's'} del análisis?`)) return;
                    ...
                    equiposExcluidosCache = await getEquiposExcluidos();
                    cerrar(); await renderPanel(); break;
                }
                case 'excluir': {
                    const motivo = prompt('¿Por qué se apartan estos equipos del análisis?\n(ej. de baja, alquilado, sin GPS instalado, en reparación)', 'sin GPS reportando en estos archivos');
                    if (motivo === null) return;
                    for (const interno of internos) await setEquipoExcluido(interno, motivo.trim() || 'apartado a mano', '');
                    equiposExcluidosCache = await getEquiposExcluidos();
                    cerrar(); await renderPanel(); break;
                }
                case 'ficha':
                    if (internos.length !== 1) { alert('Para ver la ficha, tildá un solo equipo.'); return; }
                    cerrar(); buscarEquipo(internos[0]); break;
                case 'gps':
                    if (internos.length !== 1) { alert('Para ver las filas GPS, tildá un solo equipo.'); return; }
                    cerrar();
                    if (typeof window.abrirTablaConBusqueda === 'function') window.abrirTablaConBusqueda('gps', internos[0]);
                    break;
                case 'meta':
                    if (internos.length !== 1) { alert('Para investigar la meta, tildá un solo equipo.'); return; }
                    cerrar(); abrirInvestigacionMeta(internos[0]); break;
                case 'declarar':
                    cerrar(); abrirActividadEstimada(internos, ultimoAnalisis); break;
                case 'estado':
                    cerrar(); abrirEstadoEquipoBulk(internos, hallazgoId); break;
                case 'comparar':
                    if (internos.length < 2) { alert('Para comparar, tildá al menos dos equipos.'); return; }
                ...
                case 'ralenti':
                    if (!['ralenti', 'ralenti_camionetas', 'ralenti_inverosimil'].includes(hallazgoId)) {
                        alert('Marcar ralentí aceptable solo aplica a hallazgos de ralentí.'); return;
                    }
...
function datosExportacionRevision(h, equipos, analisis, modal) {
    const seleccionados = new Set([...modal.querySelectorAll('.chk-rev:checked')].map(c => c.dataset.interno));
    return equipos.map(e => {
        const f = (analisis.filas || []).find(x => x.equipo.interno === e.interno);
        const m = f?.metrics || {};
        const meta = f?.confirmed?.valor ? `${nf(f.confirmed.valor, 2)} ${f.confirmed.unidad || m.tipo_calculo || ''}`.trim() : 'sin meta';
        const sugerida = f ? (sugerirMeta(f, analisis.filas, referentesMetaCache.get(f.equipo.interno)) || metaDesdeConsumoReal(f)) : null;
        const seg = seguimientoEquiposCache.get(e.interno);
        const atendido = (diagAtendidos.get(h.id) || new Map()).get(e.interno);
        const cob = e.cobertura;
        return {
            Seleccionado: seleccionados.has(e.interno) ? 'Sí' : 'No',
            Equipo: e.interno,
            Denominacion: e.denominacion || '',
            Hallazgo: h.titulo || h.id,
            Litros: m.total_litros || 0,
            'Cantidad de cargas': m.cantidad_cargas || 0,
            'Km GPS': m.total_km || 0,
            'Horas GPS': m.total_horas || 0,
            'Consumo medido': m.consumo_real || 0,
            'Meta actual': meta,
            'Meta sugerida': sugerida ? `${nf(sugerida.valor, 2)} ${sugerida.unidad || m.tipo_calculo || ''}`.trim() : '—',
            'Período desde': analisis.totales?.periodo_desde || '',
            'Período hasta': analisis.totales?.periodo_hasta || '',
            'Cobertura': cob?.mesesPeriodo ? `${cob.conDatos} de ${cob.mesesPeriodo} meses` : '—',
            'Estado anotado': seg ? `${seg.categoria || 'otro'}: ${seg.motivo || 'sin detalle'}` : '—',
            'Revisión': atendido ? `Atendido: ${atendido.motivo || 'revisado'}` : 'Pendiente'
        };
    });
}

function tablaExportacionRevision(titulo, rows) {
    const cols = Object.keys(rows[0] || {});
    return `<table><thead><tr>${cols.map(c => `<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>${rows.map(row =>
        `<tr>${cols.map(c => `<td>${esc(row[c] == null ? '' : row[c])}</td>`).join('')}</tr>`
    ).join('')}</tbody></table>`;
}

function exportarRevisionDecidir(formato, h, equipos, analisis, modal) {
    const rows = datosExportacionRevision(h, equipos, analisis, modal);
    if (!rows.length) { alert('No hay equipos para exportar.'); return; }
    const fecha = new Date().toISOString().slice(0, 10);
    const base = `FlotaControl_revision_${fecha}`;
    const titulo = `Revisar y decidir — ${h.titulo || h.id}`;

    if (formato === 'xlsx') {
        if (typeof XLSX === 'undefined') { alert('No se cargó el módulo de Excel.'); return; }
        const ws = XLSX.utils.json_to_sheet(rows);
        ws['!cols'] = Object.keys(rows[0]).map(c => ({ wch: Math.min(32, Math.max(12, c.length + 2)) }));
    ...
    const styles = `body{font-family:Arial,sans-serif;color:#18212b;padding:24px}h1{font-size:20px}p{color:#52606d}table{border-collapse:collapse;width:100%;font-size:10px}th,td{border:1px solid #b8c2cc;padding:5px;text-align:left;vertical-align:top}th{background:#e9eef3}`;
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${esc(titulo)}</title><style>${styles}</style></head><body><h1>${esc(titulo)}</h1><p>Exportado ${esc(fecha)} · ${rows.length} equipos · Los valores muestran el estado actual de la revisión.</p>${tablaExportacionRevision(titulo, rows)}</body></html>`;

    if (formato === 'doc') {
        const blob = new Blob([html], { type: 'application/msword;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = `${base}.doc`; a.click();
        setTimeout(() => URL.revokeObjectURL(url), 1000);
        return;
    }

    const printWindow = window.open('', '_blank');
    if (!printWindow) { alert('El navegador bloqueó la ventana de impresión. Permití ventanas emergentes para exportar a PDF.'); return; }
    printWindow.document.open();
    printWindow.document.write(html.replace('</head>', '<style>@media print{button{display:none}}</style></head>'));
 ...
 * mecanismo. Se abre desde cualquier tarjeta de hallazgo (ralentí, sin
...
function ejecutarAccionPropuesta(accion, hallazgoId, analisis) {
    ...
    // pueden leer los movimientos. Con [] esos hallazgos no aparecían acá y sus acciones no
    // hacían nada.
    ...
    switch (accion) {
        ...
        case 'revisar_parciales':
            ...
            break;
        case 'revisar_decidir':
            abrirRevisarDecidir(hallazgoId, ultimoAnalisis, datosCrudos?.rawRecords || []);
            break;
        case 'resolver':
