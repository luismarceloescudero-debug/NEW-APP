...
/** Período que debe mostrar el panel después de una carga. Equipos y metas no restringen tiempo. */
async function periodoDeArchivosSubidos(archivosSubidos) {
    const nombres = new Set(archivosSubidos.map(f => f.file.name));
    const registros = (await getAllRawRecords()).filter(r => nombres.has(r.source_file));
    const cargas = registros.filter(r => r.type === 'carga');
    const gps = registros.filter(r => r.type === 'gps');
    const periodo = periodosAnalisisAutomatico(cargas, gps);
    if (periodo.length) return periodo;
    // Si se subió una sola fuente temporal (por ejemplo, solo el GPS de julio), esa fuente
    // todavía fija el período: las otras planillas se buscan en el mismo mes desde la base.
    return [...new Set(registros.flatMap(r => r.type === 'gps' ? mesesDeRegistro(r) : (r.periodo ? [r.periodo] : [])))].sort();
}
...
async function processAllFiles() {
    ...
    if (ok > 0) {
        const periodoSubido = await periodoDeArchivosSubidos(pendientes);
        if (periodoSubido.length > 0) window.setMesesFiltro?.(periodoSubido);
        irA('panel');
    } else if (errores > 0) {
